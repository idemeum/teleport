...
type SAMLAuthResponse struct {
	...
	// the email address of the user when the external identity is SAML and idemeum connector is used
...
func (a *Server) validateSAMLResponse(ctx context.Context, diagCtx *ssoDiagContext, samlResponse string) (*SAMLAuthResponse, error) {
	...
	auth := &SAMLAuthResponse{
		...
		// when saml with idemeum is used email is always present in the saml response
		Email: attributeStatements["email"][0],
	}
	
	log.Infof("Setting the email to %v", auth.Email)
